...
namespace detail
{
    template <typename char_type, size_t... Idx>
    static constexpr auto to_array(std::index_sequence<Idx...> /*unused*/, const char_type* elems)
    {
        return std::array<char_type, sizeof...(Idx)> {
            elems[Idx]...
        };
    }
} // namespace detail
...
class string : public string_base
{
  public:
    ...
    constexpr string(const char_type (&s)[N]) { std::copy_n(s, N, elems); }

    template<size_type Ni, size_type pos = 0, size_type count = npos>
    constexpr string(const string<Ni>(&s),
                     std::integral_constant<size_type, pos>,
                     std::integral_constant<size_type, count>)
    {
        *std::copy_n(&s.elems[std::min(pos, Ni - 1)], std::min(count, Ni - 1 - std::min(pos, Ni - 1)), elems) = '\0';
    }
    ...
    constexpr auto operator+(const auto& rhs) const { return meta::string(*this, meta::string(rhs)); }

    static constexpr auto size_static() noexcept -> size_type { return N; }

    constexpr auto size() const noexcept -> size_type { return N; }

    constexpr auto empty() const noexcept -> bool { return N == 1; }

    constexpr auto data() const noexcept -> const char_type* { return elems; }

    constexpr explicit operator const char_type*() const noexcept { return elems; }

    constexpr explicit operator std::string_view() const { return std::string_view {elems, N - 1}; }

    constexpr explicit operator std::array<char_type, N>() const
    {
        return detail::to_array(std::make_index_sequence<N>(), elems);
    }
    ...
    constexpr const char_type& operator[](size_type pos) const { return at(pos); }

    constexpr const char_type& front() const noexcept { return elems[0]; }

    constexpr const char_type& back() const noexcept { return elems[N - 1]; }

    template<size_type pos = 0, size_type count = npos>
    constexpr auto substr() const
    {
        return meta::string(
            *this, std::integral_constant<size_type, pos> {}, std::integral_constant<size_type, count> {});
    }
...
template<string_constructible... T>
constexpr inline string_base::size_type string_length =
    ((decltype(string(std::declval<T>()))::size_static() - 1) + ... + 1);

}  // namespace detail

template<string_base::size_type N>
string(const string_base::char_type (&)[N]) -> string<N>;

template<string_base::size_type Ni, string_base::size_type pos, string_base::size_type count>
string(const string<Ni>(&),
       std::integral_constant<string_base::size_type, pos>,
       std::integral_constant<string_base::size_type, count>)
    -> string<std::min(count, Ni - 1 - std::min(pos, Ni - 1)) + 1>;

template<string_constructible... T>
string(const T&...) -> string<detail::string_length<T...>>;

template<template<typename...> typename container, string_constructible... T>
string(const container<T...>&) -> string<detail::string_length<T...>>;
...
}  // namespace meta_string_literals
